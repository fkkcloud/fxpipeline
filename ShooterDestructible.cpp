	...
	: Super(ObjectInitializer)
{
	...
	OriginalMeshComp->AttachTo(SceneComp);
	...
	DestroyedSkeletalMeshComp->AttachTo(SceneComp);
	...
	DestroyedStaticMeshComp->AttachTo(SceneComp);
	...
	DestructableShadowStaticMeshComp->AttachTo(SceneComp);
...
void AShooterDestructible::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	CheckRadiusComponent->AttachTo(RootComponent);
...
void AShooterDestructible::DoDamageToNearbyDestructible(AShooterDestructible* Instigator)
{
	UWorld* ActorWorld = GetWorld();
	if (!ActorWorld)
	{
...
void AShooterDestructible::RunDestruction(class AShooterProjectile* Projectile, const FHitResult & SweepResult)
{
	...
	if (GameState)
	{
		...
		DoDamageToNearbyDestructible(this);
...
void AShooterDestructible::PlayDestroySound(AShooterGameState* GameState)
{
	...
	AShooterSound* sound = GameState->AllocateSound(DestroySound);

	sound->SetActorLocation(GetActorLocation());
	sound->Play();
}
