...
void AShooterEmitter::Tick_Internal(float DeltaSeconds)
{
	...
	if ((!IsLooping && GetWorld()->TimeSeconds - SpawnTime > LifeTime) ||
		(DeathTime > 0.0f && DeathStartTime > 0.0f && GetWorld()->TimeSeconds - DeathStartTime > DeathTime))
	{
		DeallocateFromPool();
	}
}
...
void AShooterEmitter::AllocateFromPool(UParticleSystem* Template, float InLifetime, bool InIsAttachedFX, AActor* Parent, FName BoneName, float Scale)
{
	...
	if (IsAttachedFX)
	{
		AttachRootComponentToActor(Parent, BoneName, EAttachLocation::SnapToTarget);
		SetOwner(Parent);
