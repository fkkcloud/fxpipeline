...
AShooterEffectsFlipBook::AShooterEffectsFlipBook(const FObjectInitializer& ObjectInitializer)
{
	...
	if (SpriteHelperInEditor)
	{
		...
		SpriteHelperInEditor->Mobility = EComponentMobility::Static;
		...
		SpriteHelperInEditor->AttachParent = StaticMeshComp;
...
void AShooterEffectsFlipBook::AllocateFromPool(FEffectsFlipBook* FlipbookElement, AActor* Owner)
{
	...
	if (IsAttached)
	{
		...
		AttachRootComponentToActor(Owner, FlipbookElement->Bone, EAttachLocation::KeepRelativeOffset);
		SetOwner(Owner);
	}
...
void AShooterEffectsFlipBook::ResetFlipbook()
{
	...
	TeleportTo(FVector(1000000.0f, 1000000.0f, -1000.0f), FRotator::ZeroRotator);
	StaticMeshes.Empty();
